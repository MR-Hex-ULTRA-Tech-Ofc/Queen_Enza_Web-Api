...
import { version as EnzaWebVersion } from '../Defaults/EnzaWeb-version.json'
import { EnzaWebEventEmitter, EnzaWebEventMap, DisconnectReason, WACallUpdateType, WAVersion } from '../Types'
...
export const Browsers = {
	...
	EnzaWeb: browser => ['Enza', browser, '4.0.0'] as [string, string, string],
...
export function bindWaitForEvent<T extends keyof EnzaWebEventMap>(ev: EnzaWebEventEmitter, event: T) {
	return async(check: (u: EnzaWebEventMap[T]) => boolean | undefined, timeoutMs?: number) => {
		let listener: (item: EnzaWebEventMap[T]) => void
...
export const bindWaitForConnectionUpdate = (ev: EnzaWebEventEmitter) => bindWaitForEvent(ev, 'connection.update')

export const printQRIfNecessaryListener = (ev: EnzaWebEventEmitter, logger: Logger) => {
...
/**
 * utility that fetches latest EnzaWeb version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestEnzaWebVersion = async(options: AxiosRequestConfig<any> = { }) => {
	const URL = 'https://raw.githubusercontent.com/adiwajshing/EnzaWeb/master/src/Defaults/EnzaWeb-version.json'
	try {
		...
		return {
			version: EnzaWebVersion as WAVersion,
			isLatest: false,
			error
		}
	}
...
/**
 * Stream errors generally provide a reason, map that to a EnzaWeb DisconnectReason
