...
import { EnzaWebEvent, EnzaWebEventEmitter, EnzaWebEventMap, BufferedEventData, Chat, ChatUpdate, Contact, WAMessage, WAMessageStatus } from '../Types'
 ...
 */
type EnzaWebEventData = Partial<EnzaWebEventMap>

const BUFFERABLE_EVENT_SET = new Set<EnzaWebEvent>(BUFFERABLE_EVENT)

type EnzaWebBufferableEventEmitter = EnzaWebEventEmitter & {
	/** Use to process events in a batch */
	process(handler: (events: EnzaWebEventData) => void | Promise<void>): (() => void)
	/**
 ...
 * @param ev the EnzaWeb event emitter
 */
export const makeEventBuffer = (logger: Logger): EnzaWebBufferableEventEmitter => {
	...
	// take the generic event and fire it as a EnzaWeb event
	ev.on('event', (map: EnzaWebEventData) => {
	...
	return {
		process(handler) {
			const listener = (map: EnzaWebEventData) => {
				handler(map)
			}
		...
		emit<T extends EnzaWebEvent>(event: EnzaWebEvent, evData: EnzaWebEventMap[T]) {
...
function append<E extends BufferableEvent>(
	...
	case 'contacts.update':
		const contactUpdates = eventData as EnzaWebEventMap['contacts.update']
	...
	case 'messages.upsert':
		const { messages, type } = eventData as EnzaWebEventMap['messages.upsert']
	...
	case 'messages.update':
		const msgUpdates = eventData as EnzaWebEventMap['messages.update']
	...
	case 'messages.delete':
		const deleteData = eventData as EnzaWebEventMap['messages.delete']
	...
	case 'messages.reaction':
		const reactions = eventData as EnzaWebEventMap['messages.reaction']
	...
	case 'message-receipt.update':
		const receipts = eventData as EnzaWebEventMap['message-receipt.update']
		...
		break
	case 'groups.update':
		const groupUpdates = eventData as EnzaWebEventMap['groups.update']
