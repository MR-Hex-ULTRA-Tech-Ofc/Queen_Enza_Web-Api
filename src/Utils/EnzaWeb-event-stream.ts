...
import type { EnzaWebEventEmitter } from '../Types'
import { delay } from './generics'
import { makeMutex } from './make-mutex'

/**
 * Captures events from a EnzaWeb event emitter & stores them in a file
 * @param ev The event emitter to read events from
 * @param filename File to save to
 */
export const captureEventStream = (ev: EnzaWebEventEmitter, filename: string) => {
...
export const readAndEmitEventStream = (filename: string, delayIntervalMs: number = 0) => {
	const ev = new EventEmitter() as EnzaWebEventEmitter
